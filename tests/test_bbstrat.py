"""Tests for the Bollinger mid-band cross strategy (retest removed)."""
...
class SignalTests(unittest.TestCase):
    def _f15_upbreak(self):
        base = np.full(45, 100.0)                    # flat squeeze on the mid
        base[-3:] = [100.3, 100.7, 101.2]           # fresh break up + close above mid
        return _frame(base, "15min")

    def _f1h_flat(self):
        # A calm 1h series with no fresh mid-band cross, so the 15m branch wins.
        return _frame(200.0 + np.zeros(60), "60min")

    def _f5(self, last, center=101.0):
    ...
    def test_long_giris_when_5m_above_mid(self):
        # 15m broke up; 5m price above its own mid-band -> GİRİŞ long.
        sig = bb_retest_signal(self._f5(100.4, center=100.2), self._f15_upbreak(), self._f1h_flat(), BBStratParams())
    ...
    def test_long_bekle_when_5m_below_mid(self):
        ...
        sig = bb_retest_signal(self._f5(99.9, center=100.2), self._f15_upbreak(), self._f1h_flat(), BBStratParams())
        self.assertIsNotNone(sig)
        self.assertEqual(sig["side"], "long")
    ...
    def test_none_when_no_fresh_cross(self):
        ...
        f5 = self._f5(101.0)
        out = bb_retest_signal(f5, flat15, self._f1h_flat(), BBStratParams())
    ...
    def test_1h_fallback_when_no_15m_cross(self):
        ...
        f1h = _frame(h, "60min")
        sig = bb_retest_signal(self._f5(101.5), flat15, f1h, BBStratParams())
        self.assertIsNotNone(sig)
        self.assertEqual(sig["side"], "long")
